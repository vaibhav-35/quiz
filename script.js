...
function update() {
  ...
  let code = "<ul>";
  let correctans =
    questions["questions"][queIdx]["answers"][
      questions["questions"][queIdx]["correctIndex"]
    ];

  for (let i = 0; i < 4; i++) {
    if (questions["questions"][queIdx]["visited"][i] == "true") {
      if (correctans == ans[i]) code += "<li class='correctopt'>" + ans[i];
    ...
    for (let i = 0; i < 4; i++) {
      if (questions["questions"][queIdx]["answers"][i] == e.target.textContent)
        if (questions["questions"][queIdx]["visited"][i] == "false") {
          ...
          if (questions["questions"][queIdx]["answers"][i] == correctans)
            correctGuess++;
        }
    }
